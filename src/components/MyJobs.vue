...
export default {
    ...
    methods: {
        ...
        async deleteJob(id) {
            ...
            const q = query(collection(db, "works"), where("user", "==", userId));
            ...
            try {
                ...
                querySnapshot.forEach(async (docSnapshot) => {
                    const data = docSnapshot.data().work_id;
                    // Check if the document ID matches the ID to delete
                    if (data === id) {
                        // console.log("hello");
                        // Delete the document
                        await deleteDoc(doc(db, "works", docSnapshot.id));
        ...
        async getprofile() {
            ...
            // const bookingSnapshot = await getDocs(query(bookingQuery, where("user", "==", localStorage.user_id)));
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-12 mb-20" v-for="item in worksArr">
                                    ...
                                    <div class="single-job style-two"
                                        :style="{ 'pointer-events': checkStatus(item) ? 'auto' : 'none', 'opacity': checkStatus(item) ? '1' : '0.5', 'cursor': checkStatus(item) ? 'pointer' : 'not-allowed' }">
...
<template v-if="!checkStatus(item)">
                                                                ...
                                                                <div>
                                                                    <button @click="deleteJob(item.work_id)"
                                                                        style="font-size: large; background-color: red; color: white; padding: 4px 12px;">Delete</button>
                                                                    <!-- <p @onclick="deleteJob(item.id)"
                                                                            style="font-size: large; background-color: red; color: white; padding: 4px 12px;">
                                                                            Delete</p> -->
                                                                </div>
                                                            </div>
